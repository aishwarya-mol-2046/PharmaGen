...
def _cell_style(header=False):
    return ParagraphStyle(
        ...
        fontSize=8.5,
        leading=10.5,
        textColor=colors.white if header else colors.HexColor("#23271F"),
    )
...
def _evidence_table(rows) -> LongTable:
    ...
    table.setStyle(
        TableStyle(
            [
                ("BACKGROUND", (0, 0), (-1, 0), colors.HexColor("#23271F")),
                ("GRID", (0, 0), (-1, -1), 0.5, colors.HexColor("#C9C2AE")),
                ("VALIGN", (0, 0), (-1, -1), "TOP"),
                (
                    "ROWBACKGROUNDS",
                    (0, 1),
                    (-1, -1),
                    [colors.HexColor("#FBF8EF"), colors.HexColor("#F1EADA")],
                ),
...
class PDFReportService:
    ...
    def create_clinical_pdf(dataframe, meta=None) -> bytes:
        ...
        title_style = ParagraphStyle(
            ...
            fontSize=18,
            textColor=colors.HexColor("#17604F"),
        )
        section_style = ParagraphStyle(
            "Section",
            parent=styles["Heading2"],
            fontSize=13,
            textColor=colors.HexColor("#9E3315"),
            spaceBefore=14,
        )
        muted_style = ParagraphStyle(
            "Muted", parent=styles["BodyText"], fontSize=8.5, textColor=colors.HexColor("#5B5C46")
        )
        ...
        metrics.setStyle(
            TableStyle(
                [
                    ("GRID", (0, 0), (-1, -1), 0.5, colors.HexColor("#C9C2AE")),
                    ("BACKGROUND", (0, 0), (0, -1), colors.HexColor("#EEF0E4")),
        ...
        def footer(canvas, _doc):
            ...
            canvas.setFillColor(colors.HexColor("#5B5C46"))
