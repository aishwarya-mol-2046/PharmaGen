...
class KnowledgeGraphService:
    ...
    def generate_interactive_html(annotated_results: list, output_html_path="frontend/graph.html"):
        net = Network(
            height="500px", width="100%", directed=True, bgcolor="#0e1117", font_color="white"
        )
        ...
        for record in annotated_results[:10]:  # Limit top variants for clean visualization
            ...
            net.add_node(
                mut_id,
                label=f"Mut: {var['mutation']}",
                color=COLOR_MAP["Mutation"],
                shape="diamond",
                title="Somatic Variant",
            )
            ...
            for match in record["clinical_matches"][:3]:  # Top evidence only
                if match["disease"] != "No Direct Match":
                    ...
                    net.add_node(
                        disease_id, label=match["disease"], color=COLOR_MAP["Disease"], shape="box"
                    )
                    net.add_node(
                        drug_id, label=match["therapy"], color=COLOR_MAP["Drug"], shape="star"
                    )
