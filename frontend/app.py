...
st.set_page_config(page_title="PharmaGen | Evidence Console", page_icon="✦", layout="wide", initial_sidebar_state="expanded")
...
st.markdown('<div class="brand-kicker">Precision oncology / evidence console</div>', unsafe_allow_html=True)
...
if current_content:
    ...
    st.markdown(f'<div class="status-strip"><strong>Uploaded file active</strong> &nbsp; {input_filename} is being checked against the evidence base.</div>', unsafe_allow_html=True)
    if not api_base_url:
        st.error("The backend is not reachable right now. Start the API server, then re-upload the file.")
        st.stop()
    ...
    if data:
        ...
        if synthetic_data:
            st.info("Synthetic demonstration dataset: genomic coordinates are simulated; clinical relationships come from the local CIViC-derived evidence base.")
        elif data.get("exact_matches", 0) > 0:
            st.success(f"Annotated genomic input detected · {data['exact_matches']:,} exact clinical matches.")
        elif data.get("contextual_matches", 0) > 0:
            st.warning("Input parsed, but no exact gene-plus-mutation matches were found. Contextual gene evidence is shown separately.")
        else:
        ...
        matrix_columns = [
            "Gene", "Mutation", "Chromosome", "Disease", "Targeted Drug",
            "Evidence Level", "Source", "Match Type",
        ]
        ...
        with st.expander("Input quality and evidence policy"):
            ...
            quality_cols[0].metric("Valid headers", "Yes" if validation.get("valid_vcf_headers") else "No")
            ...
            quality_cols[4].metric("Annotation coverage", f"{validation.get('annotation_coverage_percent', 0)}%")
            st.markdown("**Evidence source:** local CIViC-derived clinical knowledge base. **Match policy:** exact gene plus exact mutation is actionable; gene-only evidence is contextual only; unmatched variants do not receive treatment recommendations.")
        ...
        with t1:
            st.markdown('<div class="evidence-note">Actionable treatment rows require an exact gene-plus-mutation match. Gene-context evidence is kept separate and is not a direct treatment recommendation.</div>', unsafe_allow_html=True)
            ...
            if not contextual_df.empty:
                with st.expander(f"View {len(contextual_df):,} gene-context records (not exact matches)"):
                    st.warning("These records come from other mutations in the same gene. They provide context only and must not be interpreted as a treatment recommendation for the uploaded mutation.")
            ...
            st.download_button("Download clinical review report", report_html.encode("utf-8"), file_name="pharmagen_clinical_review.html", mime="text/html", use_container_width=True)
            
        with t2:
            exact_records = [
                item for item in data["annotated_results"]
                if any(
            ...
            if not variant_options:
                st.warning("No exact gene-plus-mutation matches are available in the selected evidence tiers. Choose another tier or review the matrix.")
                st.stop()

            st.caption(f"{len(variant_options):,} matched biomarkers available in the selected evidence tiers")
            selected_variant = st.selectbox("Choose a matched biomarker to explain", variant_options)
            selected_gene, selected_mutation = [part.strip() for part in selected_variant.split(" · ", 1)]
            selected_record = next(
                item for item in exact_records
                if item["variant_info"]["gene"] == selected_gene
                and item["variant_info"]["mutation"] == selected_mutation
            )
            selected_matches = [
                match for match in selected_record["clinical_matches"]
                if match.get("match_type") == "exact" and match["evidence_tier"] in selected_levels
            ]
            selected_matches.sort(key=lambda match: (match["evidence_tier"], match["disease"], match["therapy"]))
            selected_match = selected_matches[0] if selected_matches else None
            if selected_match is None:
                st.warning("The selected biomarker has no match in the currently selected evidence tiers.")
                st.stop()
            ...
            st.caption("This focused graph explains one evidence path. It is not a diagnosis or an independent treatment recommendation.")

            with st.expander("AI-assisted clinical review (optional)"):
                st.markdown("Provide non-identifying clinical context to generate a cautious summary and context flags. The deterministic CIViC match remains the source of truth.")
                ...
                st.caption("Do not enter names, identifiers, or confidential patient data. This is review support, not a safety clearance.")
                if st.button("Generate evidence summary and safety flags", type="primary"):
                    ...
                    try:
                        ai_response = requests.post(f"{api_base_url}/api/v1/ai-review", json=ai_payload, timeout=35)
                        if ai_response.ok:
                            ai_result = ai_response.json()
                            st.session_state["ai_result"] = ai_result
                        else:
                            st.error(f"AI review could not be generated: {ai_response.text}")
                    except requests.RequestException as error:
                        st.error(f"AI review service unavailable: {error}")

                if st.session_state.get("ai_result"):
                    ai_result = st.session_state["ai_result"]
                    st.success(f"Review generated with {ai_result.get('provider', 'local-review')}.")
                    st.markdown(f"**Clinical evidence summary**\n\n{ai_result.get('summary', '')}")
                    ...
                    for flag in ai_result.get("safety_flags", []):
                        st.warning(flag)
                    st.caption(ai_result.get("disclaimer", "AI review support only; verify with a qualified professional."))
            
            net = Network(height="430px", width="100%", directed=True, bgcolor="#102f31", font_color="#eef8f4")
            ...
            COLOR_MAP = {"Gene": "#E76F51", "Mutation": "#D29A31", "Disease": "#62B7B0", "Drug": "#8CE0C3"}
            ...
            net.add_node(gene_id, label=selected_gene, title="Gene", color=COLOR_MAP["Gene"], shape="ellipse", level=0, size=28)
            net.add_node(mut_id, label=selected_mutation, title="Mutation", color=COLOR_MAP["Mutation"], shape="diamond", level=1, size=25)
            net.add_node(disease_id, label=selected_match["disease"], title="Disease association", color=COLOR_MAP["Disease"], shape="box", level=2, margin=14)
            net.add_node(drug_id, label=selected_match["therapy"], title=f"Therapy | {selected_match['evidence_tier']}", color=COLOR_MAP["Drug"], shape="star", level=3, margin=12)
